<?php	
	...
	if(isset($_POST['login'])){				
		...
		$query="select * from candidate_login where candidate_username='$username' && candidate_password='$password' && status='0'";
		$result=$con->query($query);
		if($result->num_rows>0){
			$row=$result->fetch_assoc();
			$_SESSION['candidate_id']=$row['candidate_id'];
			$_SESSION['candidate_username']=$row['candidate_username'];							
